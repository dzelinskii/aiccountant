...
**Копия правила на фронте уже есть — её надо убрать.** В
`frontend/src/pages/CategoryCell.tsx:12` стоит
`if (txn.transfer_group_id) return <Text>Перевод</Text>` — это собственное
решение фронта о том, предлагать ли выбор категории. Односторонний
`transfer_self` из импорта оно не увидит: у такой операции `transfer_group_id`
пустой. Заменить условие на `!t.counts_as_spending`, чтобы решение принималось
в одном месте.
...
тест изоляции. Вернуть. Если не краснеет — тест не проверяет изоляцию,
переписать.
