...
**Порядок (уточнён при исполнении):** сначала **Задача 2** (ledger:
`post_transaction`/`validate_posting` начинают принимать `None`), затем
**Задача 3** (дашборд), затем **Задача 1+4 вместе** (модель+миграция+схемы+
сервис recurring — единым коммитом, иначе mypy strict красный между шагами:
`rule.category_id` становится `UUID | None` и требует уже обновлённых сигнатур
`_validate`/`post_transaction`), затем **Задачи 5–6** (frontend). Задача 1
отдельно НЕ коммитится — её шаги выполняются внутри Задачи 4.
...
> **Исполняется в составе Задачи 4** (см. уточнённый порядок в шапке): модель
> и миграцию нельзя коммитить отдельно — mypy strict будет красным, пока не
> обновлены сигнатуры recurring-сервиса. Шаги ниже приведены как есть, но их
> прогон/коммит — вместе с Задачей 4.
...
### Задача 4: Опциональная категория для правил регулярки (+ миграция из Задачи 1)

> **Включает шаги Задачи 1** (модель `recurring_rules.category_id` → nullable +
> миграция `0004` + тест миграции) — они коммитятся здесь, вместе с правками
> схем/сервиса recurring, одним коммитом. Выполнять ПОСЛЕ Задачи 2.

**Файлы:**
- Modify: `backend/app/recurring/models.py` (`category_id` → nullable — из Задачи 1)
- Create: `backend/alembic/versions/0004_recurring_optional_category.py` (из Задачи 1)
...
- Test: `backend/tests/test_migrations.py`, `backend/tests/test_recurring_rules_api.py`, `backend/tests/test_recurring_beat.py`
