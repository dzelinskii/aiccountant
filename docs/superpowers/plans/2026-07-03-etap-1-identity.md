...
class OriginCheckMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if request.method in UNSAFE_METHODS and origin is not None:
            ...
            if urlparse(origin).netloc.lower() != host and origin not in get_settings().allowed_origins:
                return JSONResponse(
...
- [ ] **Step 0: Устойчивость engine и корректное закрытие ресурсов**

В `backend/app/core/db.py` добавить `pool_pre_ping=True` — после рестарта
Postgres или обрыва простаивающего соединения первый запрос через протухшее
соединение иначе упадёт вместо прозрачного переподключения:

```python
engine = create_async_engine(get_settings().database_url, pool_pre_ping=True)
```

В `backend/app/main.py` добавить lifespan, закрывающий пул БД и клиент Redis
при остановке приложения (иначе при рестартах в проде соединения закрываются
только по TCP-таймауту). Итоговый `main.py`:
...
В `.env.example` добавить строку с пояснением формата (pydantic-settings
читает список как JSON-массив, не как строку с запятыми):

```env
# ALLOWED_ORIGINS — JSON-массив, напр. ["http://localhost:5173"]
```

Прогнать `cd backend && uv run pytest -v && uv run ruff check . && uv run ruff format . && uv run mypy` — всё зелёное (lifespan не влияет на тесты через ASGITransport, но engine/redis импортируются в main).
...
```bash
git add docker-compose.yml backend/Dockerfile backend/app/core/db.py backend/app/main.py .env.example
