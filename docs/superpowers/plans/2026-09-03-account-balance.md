...
## Про `card_masks`: только присваивание

Колонка объявлена обычным `JSONB`, а не изменяемым типом. Это значит, что
присваивание нового списка (`account.card_masks = [...]`) SQLAlchemy отследит, а
правка на месте (`.append(...)`, `.remove(...)`) **потеряется молча** — запрос
на обновление просто не будет построен.

В задачах 3 и 4 список меняется только присваиванием. Если понадобится иначе —
менять тип колонки, а не надеяться на удачу.
...
**Порядок шагов — TDD:** сначала тесты (Шаг 2), затем прогон с падением, затем
...
- [ ] **Шаг 2: Снять пункт из бэклога**

В `docs/backlog.md` пункт «Остаток по счёту не соответствует действительности»
закрыт этой работой — убрать его. Ссылки на старые имена функций
(`list_accounts_with_balance`, `account_balance`) там тоже устарели: их больше
нет в коде.

- [ ] **Шаг 3: Корневой README**
...
- [ ] **Шаг 4: Живая проверка (делает владелец вместе с ассистентом)**
...
```bash
git add README.md collector/README.md docs/backlog.md
