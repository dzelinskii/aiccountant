...
# запас больше самого долгого разумного LLM-разбора (сам вызов ограничен таймаутом
# в build_llm_client — иначе дефолт SDK с ретраями мог бы приблизиться к порогу)
...
async def parse_lines(lines: list[str]) -> tuple[ParsedStatement, str]:
    ...
    return await route_statement(lines, DETERMINISTIC_PARSERS, llm)
