...
async def counterparty_names(
    ...
    У правила он стережёт обратное направление: без него чужое правило с тем же
    ключом подписало бы мою операцию чужим именем. У контрагента — связку «своё
    правило, чужой контрагент»: сервис такую не создаёт, но фильтр по операциям
    её не отсекает, а имя достаётся прямо из контрагента.
    """
