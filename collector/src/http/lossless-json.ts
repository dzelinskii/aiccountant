...
// sticky-флаг: exec с lastIndex ищет совпадение строго с позиции i, без
// вспомогательной строки-остатка. Квадратичность, вопреки первому
// впечатлению, не от него зависит: text.slice(i) на каждом символе движок
// отдаёт за O(1) (срез — представление той же строки), и тест на большом
// документе такую замену проходит. Настоящая квадратичность была в другом
// месте — см. комментарий у prev ниже. Форма (0|[1-9]\d*), а не \d+, заодно
// повторяет строгость JSON.parse: «01» числом не считается.
...
function quoteNumbers(text: string): string {
  ...
  // до непробельного символа — досканирование и давало ту самую
  // квадратичность, которую ловит тест на большом документе
  let prev = ''
  while (i < text.length) {
    const ch = text[i]!
    if (inString) {
