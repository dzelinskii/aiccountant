 ...
 * сокращаются и остаётся лимит. Замерено на живой карте, где холды были
 * ненулевыми: 1975.00 − (−51025.00) = 53000.00 при холдах 51025.00.
 *
 * Только у счёта кредитной карты: разность считается по полям, которые есть у
 * любого счёта, и без проверки признака лимит получил бы и дебетовый — число,
 * лимитом не являющееся, но от него неотличимое. Признак —
 * `properties.creditCardAccount`, а не код типа `EG`: код мы знаем с одной
 * разведки, а флаг называет ровно то, что нам нужно.
 */
