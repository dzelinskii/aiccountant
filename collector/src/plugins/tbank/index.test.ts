...
test('fetchOperations приводит операции к нашей модели', async () => {
  ...
  const operations = await fetchOperations(client, 'acc-1', 1783200000000, 1783600000000)

  expect(operations).toHaveLength(2)
  expect(operations.map((op) => op.external_id)).toEqual(['op-1', 'op-2'])
})
...
test('AUTHENTICATION_FAILED превращается в SessionExpiredError', async () => {
  const fetchImpl = vi.fn(async () => jsonResponse('{"resultCode":"AUTHENTICATION_FAILED","payload":[]}'))
  const client = createTBankClient('token', { fetchImpl: fetchImpl as unknown as FetchImpl })
...
test('иной resultCode даёт обычную ошибку, а не SessionExpiredError', async () => {
  const fetchImpl = vi.fn(async () => jsonResponse('{"resultCode":"UNKNOWN_ERROR","payload":[]}'))
  const client = createTBankClient('token', { fetchImpl: fetchImpl as unknown as FetchImpl })

  await expect(checkSession(client)).rejects.not.toBeInstanceOf(SessionExpiredError)
})

test('checkSession успешно проходит при resultCode OK', async () => {
  const fetchImpl = vi.fn(async () => jsonResponse('{"resultCode":"OK","payload":[]}'))
  const client = createTBankClient('token', { fetchImpl: fetchImpl as unknown as FetchImpl })
